...
void cb(uvc_frame_t *frame, void *ptr) {
  ...
  case UVC_COLOR_FORMAT_YUYV:
    ...
    saveBmpDataToFile(bgr->data,frame->width,frame->height,3,"grab.bmp",0);
    printf("ok\n");
}
    if (ret) {
...
int main(int argc, char **argv) {
  ...
  res = uvc_find_device(
      ctx, &dev,
      0x0120, 0x1234, NULL); /* filter devices: vendor_id, product_id, "serial_num" */
    ...
    if (res < 0) {
      ...
      if (res < 0) {
        ...
        if (res < 0) {
          ...
          const uint8_t UVC_AUTO_EXPOSURE_MODE_AUTO = 1;
          ...
          res = uvc_set_exposure_rel(devh, 9);
          res = uvc_set_exposure_rel(devh, 9);
          res = uvc_set_exposure_rel(devh, 9);
          res = uvc_set_exposure_rel(devh, 9);
          res = uvc_set_exposure_rel(devh, 9);
